...
def save_training_curves(history_csv: Path, output_dir: Path) -> None:
    ...
    plt.xlabel("Epoch")
    plt.ylabel("MSE Loss")
    plt.title("2.5D U-Net Training Loss")
    plt.grid(alpha=0.25)
    plt.legend()
    plt.tight_layout()
...
def save_metric_distributions(rows: list[dict[str, Any]], output_path: Path) -> None:
    ...
    axes[1, 0].plot([before_psnr.min(), before_psnr.max()], [before_psnr.min(), before_psnr.max()], "r--")
    ...
    axes[1, 1].plot([before_ssim.min(), before_ssim.max()], [before_ssim.min(), before_ssim.max()], "r--")
...
def save_reconstruction_samples(samples: list[dict[str, Any]], output_path: Path) -> None:
    ...
    for row, sample in enumerate(samples):
        ...
        for col, (image, title) in enumerate(panels):
            ...
            axes[row, col].imshow(image.T, cmap=cmap, origin="lower", vmin=0.0, vmax=vmax if col < 3 else None)
