...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate a trained 2.5D residual ResNet reconstruction model.")
...
def load_checkpoint(model_path: Path, device: torch.device) -> dict[str, Any]:
    ...
    return {
        ...
        "dilations": parse_dilations(DEFAULT_DILATIONS),
    }


def make_loader(dataset: BraTS25DResNetSliceDataset, args: argparse.Namespace, device: torch.device) -> DataLoader:
    return DataLoader(
        dataset,
...
def main() -> None:
    ...
    initial_output_dir = expand_path(
        args.output_dir
        or default_output_dir(args.context_slices, "nonzero", args.normalization)
    )
    model_path = expand_path(args.model_path or initial_output_dir / "best_resnet25d.pth")
    ...
    output_dir = expand_path(
        ...
        or (
            ...
            else default_output_dir(context_slices, "nonzero", normalization)
        )
    )
