...
DEFAULT_OUTPUT_DIR = BASE_DIR / "task2_densenet_deliverables_new"
...
class BraTS2DSliceDataset(Dataset):
    ...
    def __init__(self, patient_ids, undersampled_root, fully_sampled_root, 
        ...
        self.slice_filter = slice_filter # 默认 "all" 不剔除黑图
...
# ================= 3. Pure DenseNet 模型定义 =================

def _bn_function_factory(norm, relu, conv):
    def bn_function(*inputs):
        return conv(relu(norm(torch.cat(inputs, 1))))
    return bn_function
...
class PureDenseNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, num_init_features=32, growth_rate=16, block_config=(4, 4, 4, 4), drop_rate=0):
        ...
        self.final_conv = nn.Conv2d(num_features, out_channels, kernel_size=3, stride=1, padding=1)
...
def plot_loss_curve(history: list[dict[str, float]], output_dir: Path) -> None:
    ...
    ax.set_title("Pure DenseNet Training Loss")
    ax.grid(alpha=0.3)
    ax.legend()
    fig.tight_layout()
    # 【追加 _new 后缀】
...
def main():
    ...
    # 【新增参数】断点续训路径和学习率恢复
    parser.add_argument("--resume", type=str, default=None, help="Path to checkpoint (e.g. best_pure_densenet_new.pt)")
    ...
    model = PureDenseNet(in_channels=1, out_channels=1).to(device)
    ...
    if args.resume:
        ...
        if not all_patients:
            print("[FATAL ERROR] Cannot find patients in the specified directories. Please verify your absolute paths.")
            sys.exit(1)
    ...
    with log_path.open(csv_mode, newline="", encoding="utf-8") as f:
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            if val_loss < best_loss:
                best_loss = val_loss
                # 【追加 _new 后缀】
                save_path = out_dir / "best_pure_densenet_new.pt"
                # 关键：将 split 信息存入 checkpoint，方便 test 脚本识别测试集
                torch.save({
    ...
    # 训练结束后保存最后一个状态与曲线图
    # 【追加 _new 后缀】
    torch.save(model.state_dict(), out_dir / "last_pure_densenet_state_dict_new.pt")
